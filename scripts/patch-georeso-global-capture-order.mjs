...
function replaceRegex(label, regex, replacement) {
  const matches = swift.match(regex);
  if (!matches) throw new Error(`${label}: source block not found`);
...
replaceRegex(
  "remove listener-level pointer defer",
...
replaceRegex(
  "remove old document pointerdown observer",
  /      __cribloOriginalAddEventListener\.call\(document, 'pointerdown', function \(event\) \{[\s\S]*?      \}, true\);\n\n      __cribloOriginalAddEventListener\.call\(document, 'mousedown'/,
  `      __cribloOriginalAddEventListener.call(document, 'mousedown'`,
);

replaceRegex(
  "remove old document pointerup observer",
  /      __cribloOriginalAddEventListener\.call\(document, 'pointerup', function \(event\) \{[\s\S]*?      \}, true\);\n\n      __cribloOriginalAddEventListener\.call\(document, 'mouseup'/,
  `      __cribloOriginalAddEventListener.call(document, 'mouseup'`,
);

replaceRegex(
  "dispatch held pointerdown after touchstart",
...
replaceRegex(
  "dispatch held pointerup after touchend",
